...
namespace SokuLib
{
	...
	HANDLE (__stdcall * const PackageReader::findFile)(LPCSTR, LPLONG, LPLONG) = reinterpret_cast<HANDLE (__stdcall *)(LPCSTR, LPLONG, LPLONG)>(SokuLib::ADDR_PACKAGE_OPENFILE);
	...
	auto mmmInitializeReaderForEitherFileOrPackage = (bool (__thiscall *)(IFileReader **param_1, const char *filePatch))ADDR_READER_CREATE;

	IFileReader *initReader(const char *path)
	{
		IFileReader *reader = nullptr;

		if (!mmmInitializeReaderForEitherFileOrPackage(&reader, path)) {
			if (reader)
				SokuLib::Delete(reader);
			return nullptr;
		}
		return reader;
	}
